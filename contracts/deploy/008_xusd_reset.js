...
const deployName = "008_xusd_reset";
...
const deployOracles = async () => {
  ...
  const maxMinDrift = isMainnetOrFujiOrFork ? [13e7, 7e7] : [16e7, 5e7];
...
const deployVault = async () => {
  ...
  await withConfirmation(
    cVault
      .connect(sDeployer)
      .initialize(cMixOracle.address, cXUSDProxy.address, await getTxOpts())
  );
...
const upgradeAndResetXUSD = async () => {
  ...
  const propTransferGovDescription = "XUSD governance transfer";
  ...
  if (isMainnet) {
    ...
    await withConfirmation(
      cXUSDProxy
        .connect(sGovernor)
        .upgradeTo(dXUSDReset.address, await getTxOpts())
    );
    ...
    await withConfirmation(cXUSDReset.connect(sGovernor).reset());
    log("Called reset on XUSD");

    await withConfirmation(
      cXUSDProxy.connect(sGovernor).upgradeTo(dXUSD.address, await getTxOpts())
    );
...
//
// - XUSD transferGovernance to new governor
// - XUSD claimGovernance
// - XUSDProxy upgradeTo XUSDReset
// - XUSDReset reset()
// - XUSDProxy upgradeTo XUSD
