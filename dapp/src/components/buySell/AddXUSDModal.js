...
const AddXUSDModal = ({ onClose, isMobile }) => {
  const xusdAddress = useStoreState(
    ContractStore,
    (s) => s.contracts && s.contracts.xusd.address
  )
      ...
      >
        ...
        >
          <div className="d-flex justify-content-center align-items-center mb-4">
            <img className="icon" src="/images/xusd-token-icon.svg" />
          ...
          {provider !== 'metamask' && (
            <div className="contents d-flex flex-column align-items-center">
              <div className="title">
                {fbt(
                  'Track XUSD balance in your Wallet',
                  'Track XUSD in Wallet'
                )}
              </div>
              <CopyToClipboard
                onCopy={() => {
                  ...
                  {!addressCopied && (
                    <div className="d-flex address-button mt-2 justify-content-center">
                      <div className="copy-text">
                        {isMobile && (
                          <div>
                            {xusdAddress.substring(0, 21)}
                            <br />
                            {xusdAddress.substring(21)}
                          </div>
                        )}
                        {!isMobile && xusdAddress}
                      </div>
...
export default withIsMobile(AddXUSDModal)
