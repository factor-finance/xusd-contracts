...
rule preserveNonRebasingSupply {
	...
	//TODO(expect to be wrong - if the ratios are different, and I transfer to someone with a lower ratio, some credits are lost. The reverse direction is also true. The trick here is that nonRebasingSupply is denominated in XUSD rather than credits!)
